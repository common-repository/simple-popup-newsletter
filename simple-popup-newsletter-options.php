...
function simple_popup_newsletter_options() {
  ...
  echo '<h3>'.__('Setting Parameter Options', 'spn').'</h3>'.
  '<form id="popup_box_options" name="popup_box_options" method="post" action="#"><table class="tbl_wrapper">';
    ...
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Subscription and the unsubscription pop-up are active', 'spn').'</td><td> : </td><td>'.
    '<input id="option_active" name="option_active" type="checkbox" '.$option.' value="1" /></td></tr>';

    $option = "".$box_options->option_delay;
    echo '<tr><td colspan="3"><br /></td></tr>';
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Showing subscription pop-up window after this delay in milliseconds', 'spn').'</td><td> : </td><td>'.
    ...
    echo '<tr><td colspan="3"><br /></td></tr>';
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Significant part of the unsubscription URI in order to show pop-up window on this page.', 'spn').
         ' '.__('For example:').' /unsubscribe-page/ </td><td> : </td><td>'.
    ...
    echo '<tr><td colspan="3"><br /></td></tr>';
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Insert subscription button in the text editor, or copy this code into the widget text.', 'spn').
         '</td><td> : </td><td>'.
    '<input type="text" readonly="readonly" style="width: 300px;" value="'.$option.'" /></td></tr>';
    
    $option = '[unsubscribtion caption=&quot;Unsubscription&quot;]';
    echo '<tr><td colspan="3"><br /></td></tr>';
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Insert unsubscription button in the text editor, or copy this code into the widget text.', 'spn').
         '</td><td> : </td><td>'.
    '<input type="text" readonly="readonly" style="width: 300px;" value="'.$option.'" /></td></tr>';
    ...
    <form id="popup_box_form" name="popup_box_form" method="post" action="#">
      ...
      <input type="submit" value="'.__('Send', 'spn').'" />
    </form>
    ';
    ...
    echo '<tr><td colspan="3"><br /></td></tr>';
    echo '<tr><td style="text-align: right; width: 400px;">'.__('Hide pop-up windows and allow my own HTML forms like these codes below', 'spn').'</td><td> : </td><td>'.
    '<input id="option_submit" name="option_submit" type="checkbox" '.$option.' value="1" /></td></tr>';
    echo '<tr><td colspan="3"><br />'.__('Subscription HTML Form. (You can insert these in text widget for example.)', 'spn').'<br /><textarea readonly="readonly" style="background-color: #FFFFFF; width: 700px; height: 170px;">'.$code1.'</textarea></td></tr>';
    echo '<tr><td colspan="3"><br />'.__('Unsubscription HTML Form', 'spn').'<br /><textarea readonly="readonly" style="background-color: #FFFFFF; width: 700px; height: 150px;">'.$code2.'</textarea></td></tr>';
