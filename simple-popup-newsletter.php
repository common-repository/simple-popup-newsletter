...
Description: Pop-up window for email subscriptions, export email functions, include your own HTML forms and import contacts from CSV file.
Author: Varga Zsolt
...
require_once('simple-popup-newsletter-import.php');
...
add_action('init', 'spn_load_textdomain');

function spn_load_textdomain() {
  load_plugin_textdomain('spn', false, dirname(plugin_basename( __FILE__ )).'/languages'); 
}
...
function newsletter_install() {
  ...
  $sql = "CREATE TABLE ".$wpdb->prefix."newsletter_popup_subscribers (".
  ...
  $sql = "CREATE TABLE ".$wpdb->prefix."newsletter_popup_options (".
  ...
  "option_delay int(5) DEFAULT '1500' NOT NULL,".
  ...
  $sql = "INSERT INTO ".$wpdb->prefix."newsletter_popup_options (".
  "option_id, ".
...
function newsletter_menu_setup() {
	add_menu_page('Popup Newsletter', __('Popup Newsletter', 'spn'), 'manage_options', 'simple-popup-newsletter', 'newsletter_page');
  add_submenu_page('simple-popup-newsletter', 'Options', __('Options', 'spn'), 'manage_options', 'simple-popup-newsletter-options', 'simple_popup_newsletter_options'); 
  add_submenu_page('simple-popup-newsletter', 'Language', __('Language', 'spn'), 'manage_options', 'simple-popup-newsletter-langs', 'simple_popup_newsletter_langs'); 
  add_submenu_page('simple-popup-newsletter', 'Import', __('Import', 'spn'), 'manage_options', 'simple-popup-newsletter-import', 'simple_popup_newsletter_import'); 
}
...
function newsletter_footer() {
  ...
  if(isset($_POST['unsubscribe_submit'])) {
    ...
    if(!empty($email)) {
      ...
      $sql = "DELETE FROM ".$wpdb->prefix."newsletter_popup_subscribers WHERE subscriber_email = '".$email."'";
      $wpdb->query($sql);
    }
...
function subscribtion_shortcode($args, $content = null , $code = "") {
  ...
  $result = "";
  $caption = (isset($args['caption']))?$args['caption']:'Subscription';
...
function unsubscribtion_shortcode($args, $content = null , $code = "") {
  ...
  $result = "";
  $caption = (isset($args['caption']))?$args['caption']:'Unsubscription';
...
function newsletter_page() {
  ...
  if(isset($_POST['submit1'])) {
    ...
    $result = "NAME;EMAIL;"."\r\n";
    $sql = "SELECT * FROM ".$wpdb->prefix."newsletter_popup_subscribers ORDER BY subscriber_id DESC";
    $datas = array();
    $datas = $wpdb->get_results($sql);
    foreach($datas as $data) {
      $result .= "".html_entity_decode($data->subscriber_name).";".$data->subscriber_email.";"."\r\n";
    }
  ...
  if(isset($_POST['submit3'])) {
    ...
    if(!empty($sql)) {
      ...
      $sql = "DELETE FROM ".$wpdb->prefix."newsletter_popup_subscribers WHERE subscriber_id IN (".$sql.")";
      $wpdb->query($sql);
    }
  }
...
//** PAGING *************/
        ...
        $rowNumber = rowNumber("SELECT COUNT(*) AS cnt FROM ".$wpdb->prefix."newsletter_popup_subscribers ORDER BY subscriber_id DESC");
         
        $orderby = "subscriber_id DESC"; 
        $sel = isset($_GET['orderby'])?$_GET['orderby']:((isset($_POST['popup_box_table_order']))?$_POST['popup_box_table_order']:"0");
...
//** PAGING *************/
  ...
  $result = "";
  $sql = "SELECT * FROM ".$wpdb->prefix."newsletter_popup_subscribers ORDER BY ".$orderby." LIMIT ".$page.", ".ADMIN_ITEM_PER_PAGE;
  $datas = array();
  $datas = $wpdb->get_results($sql);
  foreach($datas as $data) {
    $result .= '<tr>';
  ...
  if(empty($result)) $result = '<tr><td colspan="4" style="text-align: center;">'.__('No records available!', 'spn').'</tr></td>';
...
?>
<h3><?php _e('Subscriber\'s List', 'spn'); ?></h3>
...
<input id="submit1" name="submit1" class="button button-primary" type="submit" value="<?php _e('Export .csv', 'spn'); ?>" />&nbsp;&nbsp;&nbsp;
<input id="submit2" name="submit2" class="button button-primary" type="submit" value="<?php _e('Export Only Emails', 'spn'); ?>" /><br />
<br />
<div style="float: left;">
  <select id="popup_box_table_order" name="popup_box_table_order" style="padding: 0px 10px 0px 10px;">
    <option value="0" <?php echo $op1; ?>><?php _e('Time Descending', 'spn'); ?></option>
    <option value="1" <?php echo $op2; ?>><?php _e('Email Ascending', 'spn'); ?></option>
    <option value="2" <?php echo $op3; ?>><?php _e('Name Ascending', 'spn'); ?></option>
  </select>
    <?php _e('- Order', 'spn'); ?> 
</div>
<div style="float: left; margin: 5px 0px 0px 20px;"><input id="popup_box_all_checked" name="popup_box_all_checked" type="checkbox" value="1" style="margin-top: 0px;" /> <?php _e('- Check All Boxes', 'spn'); ?></div>  
<div style="float: right;"><input id="submit3" name="submit3" class="button button-primary" type="submit" value="<?php _e('Delete Checked Emails', 'spn'); ?>" /></div>
...
<table class="wp-list-table widefat pages">
  <thead>
    <tr>
      ...
      <th><?php _e('Date', 'spn'); ?></th>
      <th><?php _e('Name', 'spn'); ?></th>
      <th><?php _e('Email', 'spn'); ?></th>
    </tr>
  </thead>
  <?php echo $result; ?>
  <tfoot>
...
function getPaging($url, $rowNumber) {
  ...
  $paging = '<div>'.(links($url, 0, '&laquo;'.__('First', 'spn'), $rowNumber));
  $paging.= links($url, $page - ADMIN_ITEM_PER_PAGE, '&laquo;'.__('Previous', 'spn'), $rowNumber);
  ...
  $paging.= links($url, $page + ADMIN_ITEM_PER_PAGE, __('Next', 'spn').'&raquo;', $rowNumber); 
  $paging.= links($url, $rowNum - ADMIN_ITEM_PER_PAGE, __('Last', 'spn').'&raquo;', $rowNumber)."</div>";
