...
function simple_popup_newsletter_import() {
  ...
  echo '<h3>'.__('Import and append from .CSV file', 'spn').'</h3>'.
  ...
  echo __('If you want, first delete the contact data which will be duplicated. Because the plugin will append the CSV list.', 'spn').'<br />';
  echo '<br />'.__('Select file', 'spn').' : <input type="file" id="file1" name="file1" accept=".csv" /><br />';
  echo __('You should use the CSV file which was exported from the Simple Popup Newsletter Plugin.', 'spn').'<br />';
  echo '<br /></td></tr></table>';
  echo '<input id="submit1" name="submit1" class="button button-primary" type="submit" value="'.__('Import file', 'spn').'" /></form>';
}
